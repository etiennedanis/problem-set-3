...
```{r}
library(stringr)
cleaned_data %>% mutate(time.value = str_replace(time.point, 'hpi', ''), time.hpi = str_c('hpi.', time.value)) %>% select(-time.point, -time.value)-> cleaned_data_hpi
cleaned_data_hpi %>% spread(time.hpi,value)
```
...
```{r}
# Using the fold ratio to determine the top 3 most consistently differentially expressed genes 
cleaned_data_hpi %>% spread(time.hpi,value) %>% mutate(expr.diff = hpi.24 / hpi.6) -> expr_data
expr_data %>% group_by(plant, fungus, gene.name) %>%   summarise(mean.hpi.6 = mean(hpi.6), mean.hpi.24 = mean(hpi.24), expr.diff.mean = mean(expr.diff), expr.diff.var=var(expr.diff)) -> mean_var_data
mean_var_data
...
```{r}
# Attempt to avoid results that does not make sense: removing  
mean_var_data %>% filter(mean.hpi.6!=0) %>% arrange(mean.hpi.6) -> filtered_mean_var_data
filtered_mean_var_data %>% filter(mean.hpi.24!=0) %>% arrange(mean.hpi.24) %>% filter(expr.diff.mean!="NaN") %>% filter(expr.diff.mean!="Inf") -> filtered_data
```
...
```{r}
# Using the fold ratio
# Previously: mean_var_data %>% arrange(desc(expr.diff.mean)) %>%  arrange(expr.diff.var) %>% slice(1:3) -> top3_genes
filtered_data %>% arrange(desc(expr.diff.mean)) %>%  arrange(expr.diff.var) %>% slice(1:3) %>% select(plant, fungus, gene.name, expr.diff.mean, expr.diff.var)-> top3_genes
# Trying to print the first 12 rows
...
The highest fold enrichment of a gene in the B12 plant expressing the immune receptor MLA1-HA treated with other effectors than the AVRA1 effector is: `r top_gene_B12_A6`.
The highest fold enrichment of a gene in the pps plant (partially immunocompromised) treated with other effectors than the AVRA1 effector is: `r top_gene_pps_A6`.
The highest fold enrichment of a gene in the B12 plant expressing the immune receptor MLA1-HA treated with the AVRA1 effector is: `r top_gene_B12_K1`.
The highest fold enrichment of a gene in the pps plant (partially immunocompromised) treated with othe AVRA1 effector is: `r top_gene_pps_K1`.
...
```{r}
library(tidyverse)
# First, create a list of the genes differentially expressed
top3_genes %>% 
cleaned_data_hpi %>% 
...
library(ggplot2)
ggplot(top3_genes, aes(x = hpi, y = value)) + geom_point() + geom_smooth(method = 'lm', se= FALSE)
