...
# Variant: cleaned_data %>% mutate(time.value = str_replace(time.point, 'hpi', ''), time.hpi = str_c('hpi.', time.value)) %>% select(-time.point, -time.value)-> cleaned_data_hpi
```
...
expr_data_mean %>% arrange(desc(expr.diff.mean)) %>%  arrange(expr.diff.var) %>% slice(1:3) %>% select(plant, fungus, gene.name, expr.diff.mean, expr.diff.var)-> top3_genes
top3_genes
...
The highest differemce im epressopm of a gene in the pps plant (partially immunocompromised) treated with other effectors than the AVRA1 effector is: `r top_gene_pps_A6`.
The highest difference of a gene in the B12 plant expressing the immune receptor MLA1-HA treated with the AVRA1 effector is: `r top_gene_B12_K1`.
The highest difference of a gene in the pps plant (partially immunocompromised) treated with othe AVRA1 effector is: `r top_gene_pps_K1`.
