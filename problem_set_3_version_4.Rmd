...
```{r question_5_4}
all_data_for_graphs$time.point <- as.numeric(all_data_for_graphs$time.point)
ggplot(all_data_for_graphs, aes(x = factor(time.point), y = value, color = gene.name, group = gene.name)) + 
  geom_point()  + xlab("time") + facet_wrap(~plant~fungus) + geom_smooth(method = 'lm', se = F)
```
...
```{r question_5_5}
all_data_for_graphs$time.point <- as.numeric(all_data_for_graphs$time.point)
ggplot(all_data_for_graphs, aes(x = factor(time.point), y = value, color = gene.name, group = gene.name, shape=factor(rep))) + 
  geom_point()  + xlab("time") + facet_wrap(~plant~fungus) +  scale_shape_manual(values=c(21,24,8)) +
...
```{r question_5_7}
# theme black and white (and theme cowplot)
library(cowplot)
all_data_for_graphs$time.point <- as.numeric(all_data_for_graphs$time.point)
ggplot(all_data_for_graphs, aes(x = factor(time.point), y = value, color = gene.name, group = gene.name, shape=factor(rep))) + 
  geom_point()  + xlab("time") + facet_wrap(~plant~fungus) + scale_shape_manual(values=c(21,24,8)) + 
