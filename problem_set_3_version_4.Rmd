...
filter(top3_genes, grepl("B12", plant)) %>%  filter(., grepl("A6", fungus)) %>%
  ungroup() %>% 
  select(expr.diff.mean) %>% 
  slice(1) %>% format(round(1), nsmall = 1) -> top_gene_B12_A6

filter(top3_genes, grepl("pps", plant)) %>%  filter(., grepl("A6", fungus)) %>% 
  ungroup() %>% 
  select(expr.diff.mean) %>% 
  slice(1) %>% format(round(1), nsmall = 1) -> top_gene_pps_A6

filter(top3_genes, grepl("B12", plant)) %>%  filter(., grepl("K1", fungus)) %>%  
  ungroup() %>% 
  select(expr.diff.mean) %>%
  slice(1) %>% format(round(1), nsmall = 1) -> top_gene_B12_K1

filter(top3_genes, grepl("pps", plant)) %>%  filter(., grepl("K1", fungus)) %>% 
  ungroup() %>% 
  select(expr.diff.mean) %>%
  slice(1) %>% format(round(1), nsmall = 1) -> top_gene_pps_K1

#top3_genes_for_B12_A6
top3_genes %>% subset(., plant =="B12" & fungus =="A6", select=c(gene.name)) %>%
  t()-> top3_genes_for_B12_A6t
top3_genes %>% subset(., plant =="B12" & fungus =="K1", select=c(gene.name)) %>%
  t()-> top3_genes_for_B12_K1t
top3_genes %>% subset(., plant =="pps" & fungus =="A6", select=c(gene.name)) %>%
  t()-> top3_genes_for_pps_A6t
top3_genes %>% subset(., plant =="pps" & fungus =="K1", select=c(gene.name)) %>%
  t()-> top3_genes_for_pps_K1t
