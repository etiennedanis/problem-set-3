...
```{r new_column}
library(stringr)
# Version 3.1: First, I calculate the mean and the variance of the replicate values for each time point and each group plant+fungus then I calculate the difference between the time point 24 hrs and the time point 6 hrs
# Then I sort first on the expression difference
# Then I sort on the variance of the replicates at 6 hrs and then on the variance of the replicates at 24 hrs
cleaned_data %>% mutate(time.point = str_replace(time.point, 'hpi', ''), time.point = str_c('hpi.', time.point))-> cleaned_data_hpi
cleaned_data_hpi %>% spread(time.point,value) -> spread_data
spread_data %>% select (gene.name, plant, fungus, rep, hpi.6, hpi.24) %>% group_by(gene.name, plant, fungus) %>% summarize(mean.hpi.6 = mean(hpi.6), mean.hpi.24 = mean(hpi.24), var.hpi.6 = var(hpi.6), var.hpi.24 = var(hpi.24)) -> mean_and_var_data
expression_data3 <- mean_and_var_data %>% mutate(expr.diff3 = abs(mean.hpi.24 - mean.hpi.6))
arranged_expr_diff3_1 <- order[expression_data3(-expr.diff3, var.hpi.6, var.hpi.24),] 
arranged_expr_diff3_1
```
```{r new_colum_alternative_approach}

# Version 3.2: First, I calculate the mean and the variance of the replicate values for each time point and each group plant+fungus then I calculate the difference between the time point 24 hrs and the time point 6 hrs
# Then I sort first on the expression difference, 
# Then I sort on the variance of the expression difference
cleaned_data %>% mutate(time.point = str_replace(time.point, 'hpi', ''), time.point = str_c('hpi.', time.point))-> cleaned_data_hpi
cleaned_data_hpi %>% spread(time.point,value) -> spread_data
spread_data %>% select (gene.name, plant, fungus, rep, hpi.6, hpi.24) %>% group_by(gene.name, plant, fungus) %>% summarize(mean.hpi.6 = mean(hpi.6), mean.hpi.24 = mean(hpi.24), var.hpi.6 = var(hpi.6), var.hpi.24 = var(hpi.24)) -> mean_and_var_data
mean_and_var_data %>% mutate(expr.diff3 = abs(mean.hpi.24 - mean.hpi.6), var.expr.diff3= var(expr.diff3)) -> expression_data3 
expression_data3 %>% arrange(desc(expr.diff3)) %>% arrange(var.expr.diff3) -> arranged_expr_diff3_2
arranged_expr_diff3_2
expression_data3 %>%  arrange(var.expr.diff3) %>% arrange(desc(expr.diff3)) -> arranged_expr_diff3_3
arranged_expr_diff3_3
```
